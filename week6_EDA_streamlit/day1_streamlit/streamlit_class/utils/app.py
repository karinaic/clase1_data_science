...
st.title('Bienvenidos al Bootcamp')
...
if menu == "Load Dataframe Columns": 
    ...
    col1, col2 = st.beta_columns([2, 4]) #esta construyendo dos columnas, la de la izquiera ocupa la mitad q la de la derecha
    slider_csv = st.sidebar.file_uploader("Select CSV", type=['csv'])
    new_df_path = None
    df_slider = None
    df_writed = None
    with col1: 
        image = Image.open(path + os.sep + 'img' + os.sep + 'happy.jpg') #carpeta happy, accedo a esa imagen, utilizando todo el tamaño de la columna
